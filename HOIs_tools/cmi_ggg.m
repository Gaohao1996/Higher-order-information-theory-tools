function I = cmi_ggg(x, y, z, biascorrect, demeaned, cov)
...
if cov
    ...
    Cxyz = shrinkage_cov(xyz); %use when needed
end
...
Cz = Cz+1e-8*eye(size(Cz));
Cxz = Cxz+1e-8*eye(size(Cxz));
Cyz = Cyz+1e-8*eye(size(Cyz));
Cxyz = Cxyz+1e-8*eye(size(Cxyz));
...
HZ = sum(log(diag(chCz)));
% + 0.5*Nvarz*log(2*pi*exp(1));
HXZ = sum(log(diag(chCxz))); 
% + 0.5*(Nvarx+Nvarz)*log(2*pi*exp(1));
HYZ = sum(log(diag(chCyz)));
% + 0.5*(Nvary+Nvarz)*log(2*pi*exp(1));
HXYZ = sum(log(diag(chCxyz)));
% + 0.5*(Nvarx+Nvary+Nvarz)*log(2*pi*exp(1));
