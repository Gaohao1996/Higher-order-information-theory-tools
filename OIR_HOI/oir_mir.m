...
% ---- 将 Geweke 频域函数值规范化为 per-Hz（单边）密度 ----
% 现在 freq 已是 0..Fs/2 (Hz)；把 f⋯ 从“频点平均值函数” → “每 Hz 密度”
f12  = f12  / Fs;
f1_2 = f1_2 / Fs;
f2_1 = f2_1 / Fs;
f1o2 = f1o2 / Fs;
...
%     F12=mean(f12);
    % ---- 频域积分得到时域标量（per-Hz 密度 → 面积）----
    F2_1 = 2 * trapz(freq, f2_1);
    F1_2 = 2 * trapz(freq, f1_2);
    F1o2 = 2 * trapz(freq, f1o2);
    F12  = 2 * trapz(freq, f12);
end
